...
export const StudentsModule: React.FC<StudentsModuleProps> = ({
  ...
  return (
    <div className="space-y-6">
          ...
          <div className={isCanViewSystemStudents ? "lg:col-span-8 space-y-4" : "w-full space-y-4"}>
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 bg-white p-4 rounded-2xl border border-slate-200/80 shadow-xs">
              ...
              <div className="flex items-center gap-2 shrink-0">
                {isManager && classes.length > 0 && onClearAllClasses && (
                  <button
                    onClick={() => setShowClearAllModal(true)}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-rose-600 bg-rose-50 hover:bg-rose-100 border border-rose-200 rounded-xl shadow-xs transition-colors cursor-pointer"
                    title="Xóa sạch toàn bộ lớp học trên hệ thống"
                  >
                    <Trash2 className="w-3.5 h-3.5" />
                    <span>Xóa tất cả ({classes.length})</span>
                  </button>
                )}
                ...
                {onOpenCreateClass && (
                  <button
                    onClick={onOpenCreateClass}
                    className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-bold text-white bg-purple-700 hover:bg-purple-800 rounded-xl shadow-xs transition-colors shrink-0 cursor-pointer"
                  >
                    <Plus className="w-3.5 h-3.5" />
                    <span>+ Tạo lớp</span>
                  </button>
      ...
      {/* Modal: Xác nhận xóa SẠCH TOÀN BỘ lớp học (Dành riêng cho Quản lý) */}
      {showClearAllModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in">
          <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl border border-rose-200 animate-in zoom-in-95">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                <h3 className="text-base font-bold text-slate-900">Xóa sạch toàn bộ {classes.length} lớp học?</h3>
                <p className="text-xs text-rose-600 font-semibold">Quyền hạn: Quản lý trung tâm (Admin)</p>
              </div>
            </div>

            <p className="text-sm text-slate-600 mb-4 leading-relaxed">
              Thao tác này sẽ xóa <strong className="text-slate-900">toàn bộ {classes.length} lớp học</strong> khỏi cơ sở dữ liệu đám mây (Firestore), bộ nhớ máy chủ (VPS) và trình duyệt. Thao tác này không thể hoàn tác.
            </p>
            ...
            <div className="bg-rose-50 border border-rose-200 rounded-xl p-3 text-xs text-rose-800 mb-5 flex items-start gap-2">
              ...
              <span>Sau khi xóa, danh sách lớp học sẽ hoàn toàn trống (0 lớp) trên toàn bộ tài khoản đăng nhập ở các máy khác.</span>
            </div>

            <div className="flex items-center justify-end gap-2.5">
              <button
                type="button"
                disabled={isClearingAllClasses}
              ...
              <button
                ...
                disabled={isClearingAllClasses}
                onClick={async () => {
                  if (!onClearAllClasses) return;
                  setIsClearingAllClasses(true);
                  try {
                    await onClearAllClasses();
                  } finally {
                    setIsClearingAllClasses(false);
